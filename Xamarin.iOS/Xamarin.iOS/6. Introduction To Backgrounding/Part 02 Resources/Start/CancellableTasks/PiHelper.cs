     ...
     */
		...
		public static string Process (int digits, Action<string> piCallback, CancellationToken token)
		{
			...
			if (digits > 0)
			{
				...
				{
					String ds = CalculatePiDigits (i + 1, token);
		...
		private static int inv_mod (int x, int y, CancellationToken token)
		{
			int q, u, v, a, c, t;
		...
		/* return (a^b) mod m */
		private static int pow_mod (int a, int b, int m, CancellationToken token)
		{
			int r, aa;
		...
		private static bool is_prime (int n, CancellationToken token)
		{
			if ((n % 2) == 0)
			{
				return false;
		...
		private static int next_prime (int n, CancellationToken token)
		{
			do
			{
				if (token.IsCancellationRequested)
			...
			while (!is_prime (n, token));

			return n;
		}

		private static String CalculatePiDigits (int n, CancellationToken token)
		{
			...
			for (int a = 3; a <= (2 * N); a = next_prime (a, token))
			{
				if (token.IsCancellationRequested)
				{
					break;
